// Lokasi: lib/features/siswa/screens/siswa_hub_screen.dart
...
import '../widgets/siswa_grid_view.dart'; // File baru yang akan kita buat
import '../../kelas/widgets/kelas_card_grid.dart';
import '../../kelas/widgets/kelas_table_view.dart'; // Tambahan
import 'siswa_form_screen.dart';
import '../../kelas/screens/kelas_form_screen.dart';

// Dialogs
import '../widgets/import_siswa_dialog.dart';
import '../widgets/bulk_plotting_dialog.dart';
import '../widgets/attendance_print_dialog.dart';
import '../widgets/siswa_card_print_dialog.dart';
...
class _SiswaHubScreenState extends ConsumerState<SiswaHubScreen> {
  ...
  bool _isKelasGridView = true;
  final TextEditingController _searchController = TextEditingController();

  // --- LOGIKA MENU ---
  void _showActionMenu() {
    final bool isSiswa = _activeTab == 0;
    showModalBottomSheet(
      ...
      builder: (context) => Column(
        ...
        children: [
          ...
          if (isSiswa) ...[
            _buildMenuTile(Icons.person_add_rounded, "Tambah Siswa Baru", () {
            ...
            _buildMenuTile(Icons.badge_rounded, "Cetak Kartu Siswa", () {
              Navigator.pop(context);
              showDialog(context: context, builder: (context) => const SiswaCardPrintDialog());
            }),
          ] else ...[
            _buildMenuTile(Icons.add_home_work_rounded, "Buat Unit Kelas", () {
              Navigator.pop(context);
              Navigator.push(context, MaterialPageRoute(builder: (context) => const KelasFormScreen()));
            }),
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      floatingActionButton: MediaQuery.of(context).size.width < 600
        ...
        label: Text(_activeTab == 0 ? "TAMBAH SISWA" : "TAMBAH KELAS",
  ...
  Widget _buildHeader() {
    return Column(
      ...
      children: [
        ...
        Text(_activeTab == 0
            ? 'Manajemen database siswa dan pencatatan hafalan.'
  ...
  Widget _buildSearchAndFilterRow() {
    return Row(
      children: [
        ...
        // Toggle View Button
        _buildIconButton(
            (_activeTab == 0 ? _isSiswaGridView : _isKelasGridView) ? Icons.view_list_rounded : Icons.grid_view_rounded,
                () {
              setState(() {
                if (_activeTab == 0) {
                  _isSiswaGridView = !_isSiswaGridView;
                } else {
                  _isKelasGridView = !_isKelasGridView;
                }
              });
            }
        ),
  ...
  Widget _buildCustomTabs() {
    return Container(
      ...
      child: Row(
        children: [
          _buildTabItem(0, 'DATABASE SISWA', Icons.people_alt_rounded),
  ...
  Widget _buildMainContent() {
    if (_activeTab == 0) {
      return _isSiswaGridView ? const SiswaGridView() : const SiswaTableView();
    } else {
